...
Ported from stratton ``src/backtest/portfolio_tracker.py`` (import path updated).
Applies portfolio-manager output as trades with commission/slippage, manages
optional stop orders, and records point-in-time snapshots.
"""
...
from app.backtest.models import HoldingSnapshot, PortfolioSnapshot, StopLossConfig, Trade
...
class PortfolioTracker:
    ...
    def _execute_buy(self, ticker: str, qty: int, price: float, trade_date: date, trigger: str) -> None:
        ...
        if ticker in self.positions:
            ...
            self.positions[ticker] = {"shares": qty, "avg_cost": price, "high_water_mark": price}
    ...
    def check_stop_orders(self, current_prices: dict[str, float], trade_date: date) -> None:
        ...
        for ticker in list(self.positions.keys()):
            ...
            if self.stop_config.stop_loss_pct:
                if price <= avg_cost * (1 - self.stop_config.stop_loss_pct):
            ...
            if self.stop_config.trailing_stop_pct:
                ...
                if price <= hwm * (1 - self.stop_config.trailing_stop_pct):
                    self._execute_sell(ticker, qty, price, trade_date, "trailing_stop")
                    continue
            if self.stop_config.take_profit_pct:
                if price >= avg_cost * (1 + self.stop_config.take_profit_pct):
    ...
    def take_snapshot(self, snap_date: date, current_prices: dict[str, float]) -> None:
        ...
        self.snapshots.append(PortfolioSnapshot(
            date=snap_date, cash=self.cash, total_value=self.cash + holdings_value, holdings=holdings,
        ))
